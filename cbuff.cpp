...
extern "C" void rem_from_cbuff(cbuffer_t* buf) {
    if (buf -> bot != buf -> top)
        buf -> bot = ((buf -> bot +1) % buf -> size);
} // as long as the cbuff isn't empty we can remove things by pointing to the bottom of the buffer and making whatever exists there equal to the next value, similar to a linked list
